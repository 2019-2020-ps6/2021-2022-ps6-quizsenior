...
export class QuizServiceDmla {
  ...
  setQuizzesFromUrl(): void {
    this.http.get<QuizDmla[]>('http://localhost:3000/api/quizzesDmla').subscribe((quizList) => {
  ...
  setSelectedQuiz(quizId: string): void {
    console.log('HERE');
    const urlWithId = 'http://localhost:3000/api/quizzesDmla/' + quizId;
  ...
  deleteQuiz(quiz: QuizDmla): void {
    ...
    const urlWithId = 'http://localhost:3000/api/quizzesDmla/' + quiz._id;
  ...
  addQuestion(quiz: QuizDmla, question: QuestionDmla): void {
    const questionUrl = 'http://localhost:3000/api/quizzesDmla/' + quiz._id + '/' + this.questionsPath;
  ...
  deleteQuestion(quiz: QuizDmla, question: QuestionDmla): void {
    const questionUrl = 'http://localhost:3000/api/quizzesDmla/' + quiz._id + '/' + this.questionsPath + '/' + question._id;
  ...
  setQuizGamesFromUrl(): void {
    this.http.get<QuizGameDmla[]>('http://localhost:3000/api/quizGames/').subscribe((quizGameList) => {
